...
from doc_processor.stamper.goods_receipt import (
    DEFAULT_CREATOR_STAMP,
    STAMP_CONFIG_CREATOR,
)


class TestGoodsReceiptStampConfigs:
    """進貨單印章配置測試"""

    def test_creator_stamp_config(self):
        """測試製表人員印章配置（基準座標與尺寸）"""
        assert STAMP_CONFIG_CREATOR.x == pytest.approx(194.0, abs=0.01)
        assert STAMP_CONFIG_CREATOR.y == pytest.approx(799.7, abs=0.01)
